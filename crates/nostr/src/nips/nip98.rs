...
#[cfg(feature = "std")]
const MAX_ENCODED_AUTH_EVENT_SIZE: usize =
    (MAX_AUTH_EVENT_SIZE / 3 + if MAX_AUTH_EVENT_SIZE % 3 == 0 { 0 } else { 1 }) * 4;
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            #[cfg(feature = "std")]
            Self::AuthTooLarge => f.write_str("authorization event too large"),
        }
    }
}
...
mod tests {
    ...
    fn oversized_auth_event_is_rejected_before_decoding() {
        ...
        assert_eq!(err, Error::AuthTooLarge);
    }
    ...
    fn maximum_sized_auth_event_reaches_json_parser() {
        ...
        assert_eq!(
            err,
            Error::Event(event::Error::Json(
                "EOF while parsing a value at line 1 column 65536".to_string()
            ))
        );
    }
